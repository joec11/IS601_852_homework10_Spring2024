...
class UserBase(BaseModel):
    ...
    full_name: Optional[str] = Field(
        None,
        max_length=100,
        description="The full name of the user, if available. Should contain only letters, spaces, hyphens, periods, or apostrophes.",
        example="John Doe"
    )
    ...
    def validate_full_name(cls, v):
        if v and not re.match(r"^[a-zA-Z\s'-.]+$", v):
            raise ValueError("Full name can only contain letters, spaces, hyphens, periods, or apostrophes.")
        return v
